...
export default function AdminPanel({
  ...
  // States for bulk participant import
  ...
  const handleBulkImport = async (e: React.FormEvent) => {
    ...
    // Split by newlines initially, or treat comma-separated names
    const lines = bulkText.split(/[\r\n]+/);
    const namesToInsert: string[] = [];

    lines.forEach(line => {
      // Handle CSV lines (take first non-empty value before comma/semicolon)
      const parts = line.split(/[;,]+/);
      const nameCandidate = parts[0]?.replace(/["']/g, "").trim();
      if (nameCandidate && nameCandidate.length > 0) {
        namesToInsert.push(nameCandidate);
      }
    });

    if (namesToInsert.length === 0) {
      setImportError("Nenhum nome válido encontrado na lista. Coloque um nome por linha ou separado por vírgula.");
      return;
    }

    setIsImporting(true);
    try {
      const imported = await firebaseImportParticipants(room.id, namesToInsert);
      ...
      setImportError(err.message || "Erro ao importar em massa.");
    } finally {
            ...
            <div className="bg-[#161920] rounded-2xl p-6 border border-white/5 lg:col-span-4 shadow-xl flex flex-col items-center text-center min-h-[520px] h-auto justify-between gap-6">
              <div>
                ...
                {window.location.origin.includes("-dev-") && (
                  <div className="mt-3 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-[10px] text-emerald-500 font-sans text-left leading-normal space-y-1">
                    <span className="font-bold flex items-center gap-1">✅ QR Code Público Habilitado</span>
                    <p>O link do QR Code foi convertido automaticamente para o link público de visualização (<strong>-pre-</strong>).</p>
                    <p className="text-slate-400">Seus participantes já podem escanear e entrar imediatamente sem nenhum erro de acesso!</p>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center gap-2">
                  {hasPermission && (
                    <button
                      ...
                      className="px-2.5 py-1 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-400 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all flex items-center gap-1 cursor-pointer hover:shadow-md"
                      title="Importar lista de participantes"
                    >
                      <Upload className="w-3 h-3" /> Importar Massa
                    </button>
                  )}
        ...
        {isQrExpanded && (
          <div className="fixed inset-0 z-[999998] flex items-center justify-center p-4">
            ...
            >
              ...
              {window.location.origin.includes("-dev-") && (
                <div className="max-w-md mt-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-[11px] text-emerald-500 font-sans text-left leading-normal space-y-1 col-span-full">
                  <span className="font-bold flex items-center gap-1">✅ QR Code Configurado no Link Público</span>
                  <p>Convertemos de forma automática o link do QR Code para o formato público (<strong>-pre-</strong>). Participantes externos podem escanear normalmente!</p>
                  <p className="text-slate-400">Gerencie o painel tranquilamente enquanto seus participantes participam utilizando o código público.</p>
                </div>
              )}
        ...
        {isImportModalOpen && (
          <div className="fixed inset-0 z-[9999999] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-[#161920] border border-white/10 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col max-h-[90vh] overflow-hidden"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsImportModalOpen(false)}
                disabled={isImporting}
                className="absolute top-4 right-4 p-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
              >
                ✕
              </button>
              ...
              <div className="flex items-center gap-3 mb-4 select-none">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5 font-sans">
                    Insira nomes de forma rápida ou carregue uma planilha
                  </p>
                </div>
              </div>

              {/* Instructions / File Upload */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 select-none shrink-0 border-b border-white/5 pb-4">
                <div className="text-xs text-slate-400 font-sans leading-relaxed text-left flex flex-col justify-center">
                  <p className="mb-2">
                    ✓ Coloque <strong className="text-slate-200">um nome por linha</strong> na caixa inferior.
                  </p>
                  <p className="mb-2">
                    ✓ Suporta arquivos <strong className="text-slate-200">CSV</strong> (nome no primeiro campo), delimitados por vírgula ou ponto-e-vírgula.
                  </p>
                  <p>
                    ✓ Os bilhetes começarão em <strong className="text-blue-400">#{101 + room.participants.length}</strong> sequencialmente.
                  </p>
                </div>
              ...
              <form onSubmit={handleBulkImport} className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                `}</style>
                <div className="flex-1 flex flex-col relative min-h-[150px] md:min-h-[220px]">
                  <textarea
                    value={bulkText}
                  ...
                  <div className="absolute bottom-3 right-3 py-1 px-2.5 bg-[#161920]/90 backdrop-blur border border-white/5 shadow-md rounded-lg font-mono text-[10px] text-blue-400 font-bold select-none">
                    Participantes Encontrados: {bulkText.split(/[\r\n]+/).map(l => l.split(/[;,]+/)[0]?.replace(/["']/g, "").trim()).filter(Boolean).length}
                  </div>
                </div>

                {importError && (
                  <div className="mt-3 text-xs text-red-400 font-bold font-sans select-none flex items-center gap-1.5 animate-pulse bg-red-500/5 border border-red-500/15 py-2 px-3 rounded-xl">
                    ⚠️ {importError}
                  </div>
                )}

                {importSuccessCount !== null && (
                  <div className="mt-3 text-[13px] text-emerald-400 font-extrabold font-sans select-none flex items-center gap-2 justify-center py-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                    ✨ Sucesso! {importSuccessCount} participantes adicionados à sala!
                  </div>
                )}

                {/* Footer buttons */}
                <div className="mt-5 flex items-center justify-end gap-3 shrink-0 select-none pt-4 border-t border-white/5">
                  <button
                    type="button"
                    onClick={() => setIsImportModalOpen(false)}
                    disabled={isImporting}
                    className="px-5 py-2.5 bg-slate-800 hover:bg-slate-750 border border-white/5 text-slate-300 hover:text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer min-w-[90px] disabled:opacity-40"
                  >
                    Voltar
                  </button>
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-black text-xs rounded-xl shadow-lg transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed min-w-[150px] justify-center"
                  >
                    {isImporting ? (
                      <>
