...
import { firebaseSendChatMessage } from "../lib/firebase";
...
export default function LiveChat({ room, playerId, playerName, isAdmin = false }: LiveChatProps) {
  ...
  return (
    <div className="bg-[#161920] border border-white/5 rounded-2.5xl shadow-xl flex flex-col h-[400px] overflow-hidden">
      {/* Chat Header */}
      <div className="bg-[#111319] px-4 py-3 border-b border-white/5 flex items-center justify-between select-none">
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg) => {
              ...
              return (
                ...
                >
                  <div className={`flex items-center gap-1 mb-1 max-w-full truncate px-1 text-[10px]`}>
                    {msg.isAdmin ? (
                  ...
                  <div
                    className={`px-3.5 py-2 rounded-2xl max-w-[85%] text-xs font-sans leading-relaxed break-words shadow-sm ${
                      isMe
      ...
      <div className="px-3 py-1.5 bg-[#111319]/85 border-t border-white/5 flex gap-1.5 overflow-x-auto justify-between shrink-0 select-none no-scrollbar">
        {emojis.map((emoji) => (
          <button
            ...
            className="text-sm px-1.5 py-0.5 hover:bg-white/5 border border-transparent hover:border-white/5 rounded-md transition-all active:scale-75 disabled:opacity-35 cursor-pointer"
          >
            {emoji}
          </button>
        ))}
      ...
      >
        <input
          ...
          maxLength={240}
          disabled={isSending}
          className="flex-1 bg-[#0F1115] border border-white/5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl px-3.5 py-2 text-xs text-slate-200 outline-none transition-all placeholder:text-slate-650"
        />
        <button
          type="submit"
